...
- Tìm hiểu cấu trúc cơ bản nhất của 1 máy tính cần có ?
  ...
  - Tìm hiểu khái niệm, Sơ đồ, Cơ chế hoạt động, Đặc điểm, Các Loại,... hệ thống (Máy tính) ?
  - Tìm hiểu 5 thành phần chính: **CPU** (Central Processing Unit ), **Memory** (RAM, ROM), **Storage** (HDD,SDD), **I/O**(Input/Output Devices), **Bus System** ?
  - Tìm hiểu về **Binary** & **Assembly basics** and **UTF**(Có bao loại UTF)
  - Tìm hiểu **ALU**, **CLU**, **Register**, **Cache**, **Cluster**
  - Tìm hiểu **Disk**; **BIOS/UEFI**; **Boot process** or **Boot Loader** là gì ?
  - Tìm hiểu nguyên lí hoạt động của 1 máy tính (Chu trình vòng lặp **Fetch**->**Decode**->**Excute**->**Repeat**)
  - Liệt kê về I/O Devices
  - So sánh **Firmware** vs **Kernel** vs **OS** (Ranh giới rõ ràng giữa từng tầng).
  - Tìm hiểu về các đơn vị **GHz**, Công nghệ sản xuất Chip hiện nay ?
